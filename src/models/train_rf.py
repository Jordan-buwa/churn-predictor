...
os.makedirs(os.path.dirname(
    train_config["logging"]["log_path"]), exist_ok=True)
logging.basicConfig(
...
target_matches = df.columns[df.columns.str.strip(
).str.lower() == TARGET_COL.lower()]
...
y = df[TARGET_COL]
logger.info(
    "Validated preprocessed data loaded successfully. Ready for training.")

# Applying SMOTETomek once before CV
apply_smotetomek = train_config.get(
    "resampling", {}).get("apply_smotetomek", False)
smote_sampler = SMOTETomek(
    random_state=train_config["model"]["random_state"]) if apply_smotetomek else None
if smote_sampler:
    X, y = smote_sampler.fit_resample(X, y)
    logger.info(
        f"SMOTETomek applied: dataset size after resampling = {X.shape[0]} samples")

# MLflow setup
if not MLFLOW_URI:
    raise ValueError("MLFLOW_TRACKING_URI not found in .env")
mlflow.set_tracking_uri(MLFLOW_URI)
# ADJUSTED: Use config value
...
def save_data_schema(X: pd.DataFrame, y: pd.Series, target_col: str, model_type: str, model_version: str):
    ...
    local_schema_path = os.path.join(
        schemas_dir, f"{model_version}_schema.json")
...
def evaluate_models(X, y, train_config):
    ...
    for name in model_names:
        ...
        with mlflow.start_run(run_name=f"{name}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"):
            ...
            mlflow.set_tag("random_state", str(
                train_config["model"]["random_state"]))
            mlflow.set_tag("preprocess_columns", ",".join(
                train_config.get("preprocessing", {}).get("drop_columns", [])))
            ...
            for train_idx, test_idx in skf.split(X, y):
                ...
                y_prob = best_model.predict_proba(X_test)[:, 1] if hasattr(
            ...
            avg_metrics = {k: np.nanmean(
                [m[k] for m in fold_metrics]) for k in fold_metrics[0]}
...
if __name__ == "__main__":
    ...
    best_model_name, best_model, best_metrics, run_id = evaluate_models(
        X, y, train_config)
    ...
    if best_model_name:
        ...
        schema, local_schema_path = save_data_schema(
            X, y, TARGET_COL, model_type, model_version_name)
        ...
        try:
            ...
            local_model_path = os.path.join(
                versions_dir, f"{model_version_name}.joblib")
            joblib.dump(best_model, local_model_path)
            logger.info(
                f"Best model saved locally (legacy) at: {local_model_path}")
        ...
        logger.info(
            f"Training completed. Best model: {best_model_name} with metrics: {best_metrics}")
