...
class SetupValidator:
    ...
    def validate_imports(self) -> bool:
        ...
        for module_name, import_stmt in import_checks:
            try:
                ...
                self.errors.append(
                    f"Import failed for {module_name}: {str(e)}")
                success = False
    ...
    def validate_model_types(self) -> bool:
        ...
        try:
            ...
            for input_type, expected in test_cases:
                ...
                if result != expected:
                    logger.error(
                        f"FAILED: Normalization failed: {input_type} -> {result} (expected {expected})")
                    self.errors.append(
                        f"Model type normalization failed for {input_type}")
                    success = False
                else:
                    logger.info(
                        f"SUCCESS: Normalization correct: {input_type} -> {result}")

            # Test validation
            valid_types = ["xgboost", "random_forest",
                           "neural_net", "random-forest", "neural-net"]
            invalid_types = ["invalid_model", "unknown", "xg_boost"]

            for model_type in valid_types:
                if validate_model_type(model_type):
                    logger.info(
                        f"SUCCESS: Validation correct for: {model_type}")
                else:
                    logger.error(
                        f"FAILED: Validation failed for valid type: {model_type}")
                    success = False

            for model_type in invalid_types:
                if not validate_model_type(model_type):
                    logger.info(
                        f"SUCCESS: Correctly rejected invalid type: {model_type}")
                else:
                    logger.error(
                        f"FAILED: Wrongly accepted invalid type: {model_type}")
                    success = False
        ...
        try:
            ...
            for prop in test_properties:
                try:
                    ...
                    self.errors.append(
                        f"Config property {prop} failed: {str(e)}")
                    return False
    ...
    def validate_ml_models(self) -> bool:
        ...
        try:
            from src.api.ml_models import get_model_path, check_models_availability
            ...
            for model_type in model_types:
                try:
                    ...
                    logger.info(
                        f"SUCCESS: Model path found for {model_type}: {path}")
                except Exception as e:
                    logger.warning(
                        f"WARNING: Could not find model path for {model_type}: {e}")
                    self.warnings.append(
                        f"Model path not found for {model_type}: {str(e)}")
