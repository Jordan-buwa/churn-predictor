...
def get_local_model_path(model_type: str) -> Path:
    ...
    if not validate_model_type(normalized_type):
        raise ValueError(
            f"Unknown model type: {model_type}. Supported: {ModelType.get_all_types()}")
    ...
    if not candidates:
        raise FileNotFoundError(
            f"No model file found for type '{model_type}' in {models_dir}")
    ...
    try:
        ...
        if model_class is not None and model_class.suffix == ".pth":
            model_class.load_state_dict(
                torch.load(path, map_location=torch.device(
            ...
            model = torch.load(path, map_location=torch.device(
                'cpu'), weights_only=False)
            model.eval()
...
def load_single_model(model_type: str, force_reload: bool = False) -> Optional[Any]:
    ...
    try:
        ...
        if normalized_type in ml_models and not force_reload:
            logger.info(
                f"Model {normalized_type} already loaded, using cached version")
