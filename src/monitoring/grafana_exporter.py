...
class SimpleGrafanaExporter:
    """
    Simple Grafana exporter using live drift monitoring.
    """
    ...
    def run_monitoring_cycle(self):
        ...
        try:
            ...
            self.logger.info(
                f"Cycle completed: {results['analysis_summary']['overall_drift_status']}"
            )
    ...
    def _start_uptime_counter(self):
        """Start uptime counter in the background"""
        def uptime_loop():
            start_time = time.time()
            while self.is_running:
...
def main():
    """Main entry point to start the exporter"""
